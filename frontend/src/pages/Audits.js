...
const CustomNoRowsOverlay = () => {
  return (
    <div className="custom-no-rows-overlay">
      ...
      <h3>No Audited Records Found</h3>
      <p>There are currently no audited call records matching your search or filters.</p>
    </div>
  );
};
...
const Audits = () => {
  ...
  const fetchAuditorStats = async () => {
    try {
      ...
      const response = await api.get(`/calls/auditors-stats?${queryParams.toString()}`);
      const data = response.data.data || [];
      setAuditorStats(data);
      window.fetchAuditorStats = fetchAuditorStats;
    } catch (error) {
  ...
  const fetchCalls = async () => {
    try {
      ...
      const queryParams = new URLSearchParams({
        ...
        search: appliedFilters.search,
        process: appliedFilters.process,
        status: 'audited', // Hardcoded status to fetch only audited calls
        dateFrom: appliedFilters.dateFrom,
        dateTo: appliedFilters.dateTo
      });
      if (selectedAuditor) {
        queryParams.append('auditorName', selectedAuditor);
      }
      ...
      console.error('Error fetching audited calls:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchCalls();
  }, [page, pageSize, appliedFilters, selectedAuditor]);
  ...
  const exportCSV = () => {
    if (gridRef.current && gridRef.current.api) {
      gridRef.current.api.exportDataAsCsv({
        fileName: `Audited_Calls_${new Date().toISOString().split('T')[0]}.csv`,
        allColumns: true
      });
    }
  };
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      ...
      console.error('Error saving audited call edits:', error);
      alert('Failed to save changes. Please try again.');
    } finally {
      setSavingEdit(false);
    }
  ...
  const columnDefs = useMemo(() => [
    ...
    { 
      ...
      editable: false,
      cellRenderer: (params) => {
        const val = params.value || 'audited';
        return (
          <select
            value={val}
            onChange={async (e) => {
  ...
  return (
    <div className="calls-container">
      <div className="calls-header">
        <div>
          <h2>Audited Calls Archive</h2>
          <p>Browse, search, and review all audited call records</p>
      ...
      {/* Auditor Statistics Cards */}
      {auditorStats.length > 0 && (
        <div className="auditor-stats-section">
          <h3>Auditor Breakdown</h3>
          <div className="auditor-cards-container">
            <div 
              className={`auditor-card-badge ${selectedAuditor === '' ? 'active' : ''}`}
              onClick={() => {
                setSelectedAuditor('');
                setPage(1);
              }}
            >
              <div className="badge-title">All Auditors</div>
              <div className="badge-count">
                {auditorStats.reduce((sum, item) => sum + item.count, 0)}
              </div>
            </div>
            
            {auditorStats.map((item) => (
              <div 
                key={item.auditorName}
                className={`auditor-card-badge ${selectedAuditor === item.auditorName ? 'active' : ''}`}
                onClick={() => {
                  setSelectedAuditor(prev => prev === item.auditorName ? '' : item.auditorName);
                  setPage(1);
                }}
              >
                <div className="badge-title">{item.auditorName}</div>
